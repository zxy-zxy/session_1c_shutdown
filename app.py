...
def handle_sessions(server_agent, cluster, base, instance):
    ...
    if not sessions:
        logging.error(
            "{} Sessions in db {} we're not found.".format(
                current_time(), base.Name.lower()
            )
        )
    ...
    for session in sessions:
        if (
            session.AppID.lower() == "backgroundjob"
            and session.userName.lower() not in instance["users_to_skip"]
        ):

            date_of_begin = datetime.datetime(
                year=session.StartedAt.year,
            ...
            if date_of_begin < datetime.datetime.now() - datetime.timedelta(hours=1):
                ...
                logging.info(
                    "{} Session with id {} for user {} from {} has been deleted".format(
                        current_time(),
                        session.SessionID,
    ...
    if not counter:
        logging.info(
            "{} In db {} no objects were found that satisfy the conditions.".format(
                current_time(), base.Name.lower()
            )
        )
...
def main():
    ...
    for instance in instances:
        ...
        if clusters is None or not clusters:
            logging.error(
                "{} Clusters for server {} not found.".format(
                    current_time(), instance["server"]
                )
            )
            break
        ...
        for cluster in clusters:
            ...
            logging.info(
                "{} Cluster {} processing was started at {}.".format(
                    current_time(), cluster.ClusterName, cluster.HostName
                )
            )
            ...
            for base in base_info:
                ...
                for found in filter(
                    lambda x: x["name"] == base.Name.lower(), instance["db"]
                ):
                    logging.info(
                        "{} Database {} processing was started.".format(
                            current_time(), base.Name.lower()
                        )
                    )
